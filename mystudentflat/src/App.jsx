  ...
  return(<><nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,background:scrolled?"rgba(255,255,255,0.96)":"rgba(255,255,255,0.9)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${scrolled?cc.border:"transparent"}`,transition:"all 0.3s",padding:scrolled?"8px 0":"14px 0"}}>
    <div style={{maxWidth:1200,margin:"0 auto",padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:28}} className="nav-desktop">
        ...
        {user?(<div style={{display:"flex",alignItems:"center",gap:10}}><div onClick={()=>navigate("profile")} style={{width:34,height:34,borderRadius:"50%",background:P,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontFamily:ff,fontWeight:700,fontSize:14,cursor:"pointer",transition:"opacity 0.2s"}} onMouseEnter={e=>e.currentTarget.style.opacity=0.8} onMouseLeave={e=>e.currentTarget.style.opacity=1} title="My Profile">{(profile?.full_name||user.email)?.[0]?.toUpperCase()||"U"}</div><button onClick={signOut} style={{...btn(false),padding:"8px 18px",fontSize:13}}>Sign Out</button></div>)
    ...
    {menuOpen&&<div style={{padding:"12px 24px 20px",background:"#fff",borderTop:`1px solid ${cc.border}`}}>
      ...
      {user?<><a onClick={()=>{navigate("profile");setMenuOpen(false);}} style={{display:"block",padding:"10px 0",fontSize:15,fontFamily:ff,color:cc.text,cursor:"pointer",borderBottom:`1px solid ${cc.border}`}}>My Profile</a><button onClick={signOut} style={{...btn(false),width:"100%",justifyContent:"center",marginTop:12}}>Sign Out</button></>:<button onClick={()=>{setShowAuth(true);setMenuOpen(false);}} style={{...btn(),width:"100%",justifyContent:"center",marginTop:12}}>Login / Sign up</button>}
    </div>}
...
function HomePage(){
  const{navigate,cities,flats}=useContext(AppContext);const[ci,setCi]=useState(0);const[cardIdx,setCardIdx]=useState(0);const[cardFade,setCardFade]=useState(true);const[heroSearch,setHeroSearch]=useState('');const[showCities,setShowCities]=useState(false);const cn=["Nicosia","Athens","Limassol"];
  const availableFlats=flats.filter(fl=>fl.status==="available"&&fl.cover_url);
  const[heroFlatIdx,setHeroFlatIdx]=useState(0);
  const heroFlat=availableFlats.length>0?availableFlats[heroFlatIdx%availableFlats.length]:null;
  useEffect(()=>{if(availableFlats.length>1){const t2=setInterval(()=>setHeroFlatIdx(i=>(i+1)%availableFlats.length),5000);return()=>clearInterval(t2);}},[availableFlats.length]);
  useEffect(()=>{const t=setInterval(()=>setCi(i=>(i+1)%cn.length),2800);return()=>clearInterval(t);},[]);
  const availFlats=flats.filter(fl=>fl.status==="available").slice(0,6);
  useEffect(()=>{if(availFlats.length<2)return;const t=setInterval(()=>{setCardFade(false);setTimeout(()=>{setCardIdx(i=>(i+1)%availFlats.length);setCardFade(true);},500);},4000);return()=>clearInterval(t);},[availFlats.length]);
  ...
  return(<div>
    ...
    <section style={{minHeight:"92vh",display:"flex",alignItems:"center",position:"relative",overflow:"hidden",background:`linear-gradient(rgba(30,27,75,0.7),rgba(30,27,75,0.7)), url('https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?w=1600&h=900&fit=crop') center/cover`}}>
      <div style={{maxWidth:1200,margin:"0 auto",padding:"140px 24px 80px",width:"100%",display:"grid",gridTemplateColumns:"1fr 1fr",gap:48,alignItems:"center"}} className="hero-grid">
        <div>
          ...
          <FadeIn delay={0.14}>
            <div style={{position:"relative",maxWidth:480}}>
              <div style={{background:"rgba(255,255,255,0.12)",backdropFilter:"blur(12px)",borderRadius:14,padding:6,display:"flex",gap:8,border:"1px solid rgba(255,255,255,0.15)"}}>
                <div style={{flex:1,position:"relative"}}>
                  <input value={heroSearch} onChange={e=>{setHeroSearch(e.target.value);setShowCities(true);}} onFocus={()=>setShowCities(true)} placeholder="Search by city or university..." style={{width:"100%",padding:"14px 18px",borderRadius:10,border:"none",background:"rgba(255,255,255,0.95)",fontFamily:ff,fontSize:14,outline:"none",color:cc.text,boxSizing:"border-box"}}/>
                  {showCities&&<div style={{position:"absolute",top:"100%",left:0,right:0,marginTop:6,background:"#fff",borderRadius:12,boxShadow:"0 12px 40px rgba(0,0,0,0.15)",border:`1px solid ${cc.border}`,overflow:"hidden",zIndex:10}}>
                    <div style={{padding:"10px 16px",borderBottom:`1px solid ${cc.border}`}}><span style={{fontFamily:ff,fontSize:12,fontWeight:600,color:cc.muted,textTransform:"uppercase",letterSpacing:"0.06em"}}>Available Cities</span></div>
                    {cities.filter(ct=>ct.is_active).filter(ct=>!heroSearch||ct.name.toLowerCase().includes(heroSearch.toLowerCase())||ct.country.toLowerCase().includes(heroSearch.toLowerCase())).map(ct=>(
                      <div key={ct.id} onClick={()=>{setHeroSearch(ct.name);setShowCities(false);navigate("listings",{city:ct.id});}} style={{padding:"12px 16px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",transition:"background 0.15s",borderBottom:`1px solid ${cc.border}`}} onMouseEnter={e=>e.currentTarget.style.background=cc.light} onMouseLeave={e=>e.currentTarget.style.background="#fff"}>
                        <div><div style={{fontFamily:ff,fontSize:14,fontWeight:600,color:cc.text}}>{ct.name}</div><div style={{fontFamily:ff,fontSize:12,color:cc.muted}}>{ct.country}</div></div>
                        <ChevronRight size={16} color={cc.muted}/>
                      </div>
                    ))}
                    {cities.filter(ct=>!ct.is_active).length>0&&<div style={{padding:"10px 16px",borderTop:`1px solid ${cc.border}`}}><span style={{fontFamily:ff,fontSize:12,color:cc.muted,fontStyle:"italic"}}>Coming soon: {cities.filter(ct=>!ct.is_active).map(ct=>ct.name).join(", ")}</span></div>}
                  </div>}
                </div>
                <button onClick={()=>{setShowCities(false);navigate("listings",{search:heroSearch});}} style={{padding:"14px 28px",borderRadius:10,border:"none",background:P,color:"#fff",fontFamily:ff,fontSize:14,fontWeight:600,cursor:"pointer",transition:"all 0.2s",whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:6}}><Search size={16}/>Browse</button>
              </div>
              {showCities&&<div onClick={()=>setShowCities(false)} style={{position:"fixed",inset:0,zIndex:5}}/>}
            </div>
          </FadeIn>
        ...
        <FadeIn delay={0.08} className="hero-img">{heroFlat&&<div onClick={()=>navigate("flat",{id:heroFlat.id})} style={{borderRadius:20,overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,0.3)",aspectRatio:"4/5",position:"relative",cursor:"pointer",transition:"transform 0.3s"}} onMouseEnter={e=>e.currentTarget.style.transform="scale(1.02)"} onMouseLeave={e=>e.currentTarget.style.transform=""}><img src={heroFlat.cover_url} alt={heroFlat.title} style={{width:"100%",height:"100%",objectFit:"cover",transition:"opacity 0.5s"}} key={heroFlat.id}/><div style={{position:"absolute",inset:0,background:"linear-gradient(180deg,transparent 50%,rgba(0,0,0,0.4) 100%)"}}/><div style={{position:"absolute",bottom:20,left:20,right:20,background:"rgba(255,255,255,0.95)",backdropFilter:"blur(12px)",borderRadius:12,padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontFamily:ff,fontWeight:600,fontSize:14,color:cc.text}}>{heroFlat.title}</div><div style={{fontFamily:ff,fontSize:12,color:cc.muted}}>{heroFlat.cities?.name||"Available"} · Available Now</div></div><div style={{fontFamily:fs,fontSize:20,color:P}}>€{heroFlat.price_per_month}<span style={{fontSize:12,color:cc.muted,fontFamily:ff}}>/mo</span></div></div></div>}</FadeIn>
      </div>
    </section>
    ...
    {/* LISTINGS + CITIES */}
    <section style={{padding:"90px 24px",background:cc.light}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <FadeIn><div style={{textAlign:"center",marginBottom:48}}>
          <h2 style={{fontFamily:fs,fontSize:"clamp(24px,3vw,36px)",color:cc.text,margin:"0 0 4px",textTransform:"uppercase",letterSpacing:"0.03em"}}>Explore Our Properties</h2>
          <p style={{fontFamily:ff,fontSize:15,color:cc.muted,fontStyle:"italic"}}>Browse flats and choose your city</p>
        </div></FadeIn>
        <div style={{display:"grid",gridTemplateColumns:"1fr 340px",gap:28,alignItems:"start"}} className="listings-cities-grid">
          {/* Left: Scrollable Listings */}
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <h3 style={{fontFamily:fs,fontSize:18,color:cc.text,margin:0}}>Most Viewed</h3>
              <button onClick={()=>navigate("listings")} style={{fontFamily:ff,fontSize:13,color:P,background:"none",border:"none",cursor:"pointer",fontWeight:600}}>View All</button>
            </div>
            <div style={{maxHeight:520,overflowY:"auto",paddingRight:8,display:"flex",flexDirection:"column",gap:14}}>
              {flats.filter(fl=>fl.status==="available").slice(0,8).map((fl,i)=>(
                <div key={fl.id} onClick={()=>navigate("flat",{id:fl.id})} style={{display:"flex",gap:16,padding:12,borderRadius:14,background:"#fff",border:`1px solid ${cc.border}`,cursor:"pointer",transition:"all 0.25s",flexShrink:0}} onMouseEnter={e=>{e.currentTarget.style.boxShadow="0 8px 24px rgba(0,0,0,0.06)";e.currentTarget.style.borderColor=P;}} onMouseLeave={e=>{e.currentTarget.style.boxShadow="";e.currentTarget.style.borderColor=cc.border;}}>
                  <img src={fl.cover_url||"https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?w=300&h=200&fit=crop"} alt={fl.title} style={{width:140,height:100,objectFit:"cover",borderRadius:10,flexShrink:0}}/>
                  <div style={{display:"flex",flexDirection:"column",justifyContent:"space-between",flex:1,minWidth:0}}>
                    <div>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                        <h4 style={{fontFamily:fs,fontSize:15,color:cc.text,margin:0,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{fl.title}</h4>
                        <Tag color={cc.green} bg="rgba(34,197,94,0.1)">Available</Tag>
                      </div>
                      <p style={{fontFamily:ff,fontSize:12,color:cc.muted,margin:"4px 0 0",display:"flex",alignItems:"center",gap:4}}><MapPin size={11}/>{fl.cities?.name}, {fl.cities?.country}</p>
                    </div>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      ...
                      <span style={{fontFamily:fs,fontSize:18,color:P,fontWeight:600}}>€{fl.price_per_month}<span style={{fontSize:10,color:cc.muted,fontFamily:ff,fontWeight:400}}>/mo</span></span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {/* Right: Cities */}
          <div>
            <h3 style={{fontFamily:fs,fontSize:18,color:cc.text,margin:"0 0 16px"}}>Choose Your City</h3>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              {cities.map((ct,i)=>(
                <div key={ct.id} onClick={()=>ct.is_active&&navigate("listings",{city:ct.id})} style={{padding:20,borderRadius:14,background:ct.is_active?Pd:"#fff",border:`1px solid ${ct.is_active?"transparent":cc.border}`,cursor:ct.is_active?"pointer":"default",transition:"all 0.3s",opacity:ct.is_active?1:0.5}} onMouseEnter={e=>{if(ct.is_active)e.currentTarget.style.transform="translateY(-2px)";}} onMouseLeave={e=>{if(ct.is_active)e.currentTarget.style.transform="";}}>
                  <h4 style={{fontFamily:fs,fontSize:18,color:ct.is_active?"#fff":cc.text,margin:"0 0 4px"}}>{ct.name}</h4>
                  <p style={{fontFamily:ff,fontSize:13,color:ct.is_active?"rgba(255,255,255,0.6)":cc.muted,margin:0}}>{ct.is_active?"View flats →":"Coming Soon"}</p>
                </div>
              ))}
            </div>
          </div>
    ...
    <section style={{padding:"90px 24px",background:"#fff"}}><div style={{maxWidth:1200,margin:"0 auto"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(300px, 1fr))",gap:20}} className="reviews-grid">{REVIEWS.map((r,i)=>(<FadeIn key={i} delay={i*0.06}><div style={{padding:28,borderRadius:16,background:"#fff",border:`1px solid ${cc.border}`,height:"100%"}}>
...
function ListingsPage({initialCity,initialSearch}){
  const{flats,cities,navigate}=useContext(AppContext);const[cf,setCf]=useState(initialCity||"all");const[pr,setPr]=useState("all");const[bf,setBf]=useState("all");const[search,setSearch]=useState(initialSearch||"");
...
function FlatDetailPage({flatId}){
  ...
  const maxDate=(()=>{const d=new Date();d.setDate(d.getDate()+16);return d.toISOString().split("T")[0];})();
  const availDates=(()=>{const dates=[];for(let i=0;i<=16;i++){const d=new Date();d.setDate(d.getDate()+i);dates.push(d.toISOString().split("T")[0]);}return dates;})();
  ...
  return(<div style={{paddingTop:80,minHeight:"100vh",background:"#fff"}}>{showAuth&&<AuthModal onClose={()=>setShowAuth(false)}/>}<div style={{maxWidth:1200,margin:"0 auto",padding:"32px 24px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 380px",gap:32,alignItems:"start"}} className="detail-grid">
      ...
      <FadeIn delay={0.08}><div className="booking-card" style={{position:"sticky",top:100,borderRadius:18,background:"#fff",border:`1px solid ${cc.border}`,boxShadow:"0 4px 20px rgba(0,0,0,0.04)",overflow:"hidden"}}>
        ...
        <div style={{padding:"8px 12px",borderRadius:8,border:`1px solid #c7d2fe`,background:"#eef2ff",marginBottom:8,display:"flex",justifyContent:"space-between"}}><span style={{fontFamily:ff,fontSize:12,color:P,fontWeight:600}}>Available dates:</span><span style={{fontFamily:ff,fontSize:12,color:cc.text}}>{new Date(today).toLocaleDateString("en-GB",{day:"numeric",month:"short"})} - {new Date(maxDate).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})} (16 days)</span></div>
        <select value={mid} onChange={e=>setMid(e.target.value)} style={{...inp,marginBottom:6,cursor:"pointer",appearance:"auto"}}><option value="">Select a date...</option>{availDates.map(d=><option key={d} value={d}>{new Date(d+"T12:00:00").toLocaleDateString("en-GB",{weekday:"short",day:"numeric",month:"short",year:"numeric"})}</option>)}</select>
        ...
        {showDepositInfo&&<Modal onClose={()=>setShowDepositInfo(false)}><h3 style={{fontFamily:fs,fontSize:20,color:cc.text,margin:"0 0 16px"}}>Security Deposit Information</h3><p style={{fontFamily:ff,fontSize:15,lineHeight:1.7,color:cc.muted,margin:0}}>The amount of the security deposit is refundable at the end of the agreed rental period, provided that all utility bills have been settled in full by the tenant and that no damages have occurred to the flat during the rental period. The security deposit and the platform fee are non-refundable in case of cancellation of the rental agreement.</p></Modal>}
        {showFeeInfo&&<Modal onClose={()=>setShowFeeInfo(false)}><h3 style={{fontFamily:fs,fontSize:20,color:cc.text,margin:"0 0 16px"}}>Platform Fee Information</h3><p style={{fontFamily:ff,fontSize:15,lineHeight:1.7,color:cc.muted,margin:0}}>Platform fee is non-refundable.</p></Modal>}
        ...
        {mid&&<div style={{marginTop:14}}>
          ...
          <div onClick={()=>setPayOption("full")} style={{padding:14,borderRadius:10,border:`1px solid ${payOption==="full"?P:cc.border}`,background:payOption==="full"?"#eef2ff":"#fff",cursor:"pointer",marginBottom:14}}>
            ...
            <div style={{fontFamily:ff,fontSize:13,color:cc.muted}}>{(flat.price_per_month*parseInt(ld)+2000).toLocaleString()}€</div>
          </div>
        </div>}
      ...
      {(flat.latitude&&flat.longitude)&&<FadeIn delay={0.12}><div style={{borderRadius:16,overflow:"hidden",border:`1px solid ${cc.border}`,background:"#fff",marginTop:20}}>
        <div style={{padding:"12px 18px",borderBottom:`1px solid ${cc.border}`,display:"flex",alignItems:"center",gap:8}}><MapPin size={16} color={P}/><span style={{fontFamily:ff,fontSize:14,fontWeight:600,color:cc.text}}>Location</span></div>
        <div style={{height:300}}>
          <iframe src={`https://maps.google.com/maps?q=${flat.latitude},${flat.longitude}&z=16&output=embed`} style={{width:"100%",height:"100%",border:"none"}} loading="lazy" allowFullScreen/>
        </div>
        <div style={{padding:"10px 18px"}}><a href={`https://www.google.com/maps?q=${flat.latitude},${flat.longitude}`} target="_blank" rel="noopener noreferrer" style={{display:"inline-flex",alignItems:"center",gap:6,fontFamily:ff,fontSize:13,color:P,textDecoration:"none",fontWeight:500}}><MapPin size={14}/>View on Google Maps</a></div>
      </div></FadeIn>}
    </div>)}
  </div><Footer/></div>);
}
...
function AdminPage(){
  ...
  const stats=[{l:"Total Flats",v:flats.length,i:"home"},{l:"Available",v:flats.filter(fl=>fl.status==="available").length,i:"check"},{l:"Bookings",v:bookings.length,i:"file"},{l:"Pending",v:bookings.filter(b=>b.status==="pending").length,i:"clock"}];
  ...
  const TH=({children})=><th style={{padding:"12px 16px",textAlign:"left",fontSize:12,fontWeight:700,color:cc.muted,textTransform:"uppercase",letterSpacing:"0.06em"}}>{children}</th>;
    ...
    <section style={{padding:"80px 24px",background:cc.light}}><div style={{maxWidth:1000,margin:"0 auto"}}>
      ...
      <FadeIn delay={0.06}>
        <div style={{background:"#fff",borderRadius:16,border:`1px solid ${cc.border}`,overflow:"hidden"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0}} className="invest-grid">
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,padding:4}} className="invest-images">
...
function ProfilePage(){
  const{user,profile,setProfile,showToast,navigate}=useContext(AppContext);
  ...
  if(!user)return<div style={{paddingTop:140,textAlign:"center"}}><h2 style={{fontFamily:fs,fontSize:28,color:cc.text}}>Please sign in</h2><p style={{fontFamily:ff,color:cc.muted,marginTop:8}}>You need to be logged in to view your profile.</p></div>;

  const handleSaveProfile=async()=>{
    if(!name.trim()){showToast("Name is required","error");return;}
    setSaving(true);
    try{
      await supabase.update("profiles",user.id,{full_name:name.trim(),phone:phone.trim()});
      setProfile(prev=>({...prev,full_name:name.trim(),phone:phone.trim()}));
  ...
  const handleChangePassword=async()=>{
    ...
    try{
      const res=await fetch(`${supabase.url}/auth/v1/user`,{
        method:"PUT",
        headers:{"apikey":supabase.key,"Authorization":`Bearer ${supabase.token}`,"Content-Type":"application/json"},
        body:JSON.stringify({password:newPw})
      });
      const data=await res.json();
      if(data.error)throw new Error(data.error.message||"Password change failed");
  ...
  return(<div style={{paddingTop:80,minHeight:"100vh",background:cc.light}}><div style={{maxWidth:700,margin:"0 auto",padding:"40px 24px"}}>
    <FadeIn>
      <h1 style={{fontFamily:fs,fontSize:"clamp(24px,3vw,32px)",color:cc.text,margin:"0 0 8px"}}>My Profile</h1>
      <p style={{fontFamily:ff,fontSize:15,color:cc.muted,margin:"0 0 32px"}}>Manage your account details</p>
    </FadeIn>

    {/* Profile Info Card */}
    <FadeIn delay={0.06}>
      <div style={{padding:28,borderRadius:16,background:"#fff",border:`1px solid ${cc.border}`,marginBottom:20}}>
        <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:24,paddingBottom:20,borderBottom:`1px solid ${cc.border}`}}>
          <div style={{width:56,height:56,borderRadius:"50%",background:P,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontFamily:ff,fontWeight:700,fontSize:22}}>{(profile?.full_name||user.email)?.[0]?.toUpperCase()||"U"}</div>
          <div>
            <div style={{fontFamily:ff,fontSize:18,fontWeight:600,color:cc.text}}>{profile?.full_name||"User"}</div>
            <div style={{fontFamily:ff,fontSize:14,color:cc.muted}}>{user.email}</div>
            <div style={{fontFamily:ff,fontSize:12,color:P,fontWeight:500,marginTop:2,textTransform:"capitalize"}}>{profile?.role||"student"} account</div>
          </div>
        </div>
        ...
        <div style={{marginBottom:20}}>
          ...
          <input value={phone} onChange={e=>setPhone(e.target.value)} style={inp} placeholder="+357 or +30..."/>
        </div>

        <div style={{marginBottom:20}}>
          <label style={lbl}>Email</label>
          <div style={{...inp,background:cc.light,color:cc.muted,cursor:"not-allowed"}}>{user.email}</div>
          <p style={{fontFamily:ff,fontSize:12,color:cc.muted,marginTop:4}}>Email cannot be changed</p>
        </div>

        <button onClick={handleSaveProfile} disabled={saving} style={{...btn(),padding:"12px 32px",borderRadius:10,opacity:saving?0.6:1}}>{saving?"Saving...":"Save Changes"}</button>
      </div>
    </FadeIn>
    ...
    <FadeIn delay={0.1}>
      <div style={{padding:28,borderRadius:16,background:"#fff",border:`1px solid ${cc.border}`,marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:showPwForm?20:0}}>
          <div>
            <h3 style={{fontFamily:fs,fontSize:18,color:cc.text,margin:0}}>Password</h3>
            {!showPwForm&&<p style={{fontFamily:ff,fontSize:13,color:cc.muted,margin:"4px 0 0"}}>Change your account password</p>}
          </div>
          {!showPwForm&&<button onClick={()=>setShowPwForm(true)} style={{...btn(false),padding:"8px 20px",fontSize:13}}>Change Password</button>}
        </div>

        {showPwForm&&<div>
          <div style={{marginBottom:16}}>
            <label style={lbl}>New Password</label>
            <input type="password" value={newPw} onChange={e=>setNewPw(e.target.value)} style={inp} placeholder="Minimum 6 characters"/>
          </div>
          <div style={{marginBottom:20}}>
            <label style={lbl}>Confirm New Password</label>
            <input type="password" value={confirmPw} onChange={e=>setConfirmPw(e.target.value)} style={inp} placeholder="Re-enter new password"/>
          </div>
          <div style={{display:"flex",gap:10}}>
            <button onClick={handleChangePassword} disabled={changingPw} style={{...btn(),padding:"10px 28px",borderRadius:10,opacity:changingPw?0.6:1}}>{changingPw?"Changing...":"Update Password"}</button>
            <button onClick={()=>{setShowPwForm(false);setNewPw("");setConfirmPw("");}} style={{...btn(false),padding:"10px 28px",borderRadius:10}}>Cancel</button>
          </div>
        </div>}
      </div>
    </FadeIn>
    ...
    {/* My Bookings Card */}
    <FadeIn delay={0.14}>
      <div style={{padding:28,borderRadius:16,background:"#fff",border:`1px solid ${cc.border}`}}>
        <h3 style={{fontFamily:fs,fontSize:18,color:cc.text,margin:"0 0 16px"}}>My Bookings</h3>
...
function MyBookings({userId}){
  ...
  useEffect(()=>{(async()=>{try{const bks=await supabase.query("bookings",{select:"*,flats(title,address)",filters:[`tenant_id=eq.${userId}`],order:"created_at.desc"});setBookings(bks);}catch{}setLoading(false);})();},[userId]);
  if(loading)return<p style={{fontFamily:ff,fontSize:14,color:cc.muted}}>Loading bookings...</p>;
  if(bookings.length===0)return<p style={{fontFamily:ff,fontSize:14,color:cc.muted}}>You have no bookings yet. Browse our flats to get started!</p>;
  return(<div style={{display:"flex",flexDirection:"column",gap:12}}>{bookings.map(b=>(
    <div key={b.id} style={{padding:16,borderRadius:12,background:cc.light,border:`1px solid ${cc.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
      <div>
        <div style={{fontFamily:ff,fontSize:15,fontWeight:600,color:cc.text}}>{b.flats?.title||"Flat"}</div>
        <div style={{fontFamily:ff,fontSize:13,color:cc.muted}}>{b.move_in_date} — {b.move_out_date} · {b.lease_duration} months</div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <span style={{fontFamily:ff,fontSize:15,fontWeight:600,color:P}}>€{b.monthly_rent}/mo</span>
...
function TermsPage(){
  const Sec=({children})=><h2 style={{fontFamily:fs,fontSize:18,color:cc.text,margin:"28px 0 12px"}}>{children}</h2>;
  const Para=({children})=><p style={{fontFamily:ff,fontSize:14,lineHeight:1.8,color:cc.text,margin:"0 0 12px"}}>{children}</p>;
  ...
  return(<div style={{paddingTop:80,minHeight:"100vh",background:"#fff"}}><div style={{maxWidth:800,margin:"0 auto",padding:"60px 24px"}}>
    ...
    <div style={{padding:20,borderRadius:12,background:cc.light,border:`1px solid ${cc.border}`,marginBottom:24}}>
      <Para><strong>Legal Notice</strong></Para>
      <Para>Legal entities operating under the "MY STUDENT FLAT" brand in Cyprus and Greece:</Para>
      <Para><strong>Cyprus</strong><br/>REPURPOSE TRAVEL & REALTY LTD<br/>95 ARCH MAKARIOS AVENUE<br/>1070 NICOSIA, CYPRUS</Para>
      <Para><strong>Greece</strong><br/>REPURPOSE TRAVEL & REALTY GR M.I.K.E.<br/>3 LEKKA STREET, ATHENS 10563, GREECE</Para>
    </div>
    <Para>The terms and conditions set out in this document ("Terms") together with the confirmation email ("Confirmation Email") provided to you for leasing the Property from the Company form the contract between you, the Guest, and the Company ("Booking").</Para>
    <Para><strong>By booking any property with MY STUDENT FLAT you accept the following terms & conditions:</strong></Para>

    <Sec>1. Payment & Security Deposit</Sec>
    ...
    <Sec>3. Early Termination & Lease Extension</Sec>
    ...
    <Sec>4. Cleaning & Maintenance</Sec>
    ...
    <div style={{padding:16,borderRadius:10,background:"#fef3c7",border:"1px solid #fde68a",marginTop:24}}>
      <Para><strong>* The advertised prices are for a 12-month lease on full prepayment basis.</strong></Para>
      <Para><strong>* Cash payments exceeding 500 Euro are not accepted.</strong></Para>
      <Para>Utility costs (Wi-Fi, fees, electricity, water, heating) are estimated and not included in rent. Utility prepayment is adjusted at lease end based on actual use.</Para>
    </div>
    </FadeIn>
  </div><Footer/></div>);
}
...
function PrivacyPage(){
  const Sec2=({children})=><h2 style={{fontFamily:fs,fontSize:18,color:cc.text,margin:"28px 0 12px"}}>{children}</h2>;
  const Para2=({children})=><p style={{fontFamily:ff,fontSize:14,lineHeight:1.8,color:cc.text,margin:"0 0 12px"}}>{children}</p>;
  return(<div style={{paddingTop:80,minHeight:"100vh",background:"#fff"}}><div style={{maxWidth:800,margin:"0 auto",padding:"60px 24px"}}>
    <FadeIn>
    ...
    <Para2>This Privacy and Cookie Statement of REPURPOSE TRAVEL & REALTY LTD applies to you when you register as a house seeker, respond to our housing offer, rent a living space with us, use our service portal, or visit our website www.mystudentflat.com or contact us.</Para2>

    <div style={{padding:20,borderRadius:12,background:cc.light,border:`1px solid ${cc.border}`,marginBottom:24}}>
      <Para2><strong>Contact</strong></Para2>
      <Para2>REPURPOSE TRAVEL & REALTY LTD<br/>25 Aphroditis Street<br/>1060 Nicosia, CYPRUS<br/>info@mystudentflat.com<br/>+357 96237867</Para2>
    </div>

    <Sec2>Personal Data That We Process</Sec2>
    <Para2><strong>Flat Seekers:</strong> To book a flat through our website, you must register as a house seeker. We ask you to fill in personal details including your first name, last name, gender, date of birth, type of study, language, address, telephone number and living preferences. You create a password to log in using your email address. We record when you booked and which flat it concerns. We may also record communication between you and our employees.</Para2>
    <Para2><strong>Tenants:</strong> If you rent a flat from us, we may also process: date of your rental contract, additional services you use, repair requests, correspondence, rent payments, data about rent arrears, keys given, log data of digital keys, complaints and mediation data.</Para2>
    <Para2><strong>Newsletters:</strong> We process your email address to send newsletters about available flats matching your preferences.</Para2>
    <Para2><strong>Website Users:</strong> We may record data about the device and browser you use (device type, operating system, browser type, IP address). We use cookies as described below.</Para2>

    <Sec2>Purpose of Processing</Sec2>
    <Para2>Your personal data is processed for: offering matching flats and services, contacting you about bookings, executing lease contracts, collecting payments, offering additional services, sending updates and newsletters, handling requests and complaints, carrying out maintenance, complying with legislation, enforcing house rules, improving our services, and protecting our operations.</Para2>

    <Sec2>Legal Basis</Sec2>
    <Para2>We process personal data based on: execution of an agreement, legal obligation, legitimate interest, or your consent. You can withdraw consent at any time by contacting us.</Para2>

    <Sec2>Disclosure to Third Parties</Sec2>
    <Para2>We may provide your data to third parties when necessary to provide services, including IT and cloud service providers, website hosts, newsletter service providers, property owners or managers, external service providers, and competent authorities when legally required.</Para2>

    <Sec2>Data Retention</Sec2>
    <Para2>We process your data as long as you are registered as a flat seeker or have a rental agreement. After unsubscribing, data is stored for maximum 1 year. After lease cancellation, lease data is stored for at least 3 years. Other data may be stored for 2 years unless legally required otherwise.</Para2>

    <Sec2>Data Security</Sec2>
    <Para2>We take the protection of your data seriously with appropriate measures including security software, secure internet connections, and restricted access to authorized personnel only. If you believe your data is not well secured, contact us at info@mystudentflat.com.</Para2>

    <Sec2>Your Rights</Sec2>
    <Para2>You have the right to access, correct, or delete your personal data. You can revoke consent, object to processing, or request data transfer. Send requests to info@mystudentflat.com. We will respond within four weeks.</Para2>

    <Sec2>Cookie Statement</Sec2>
    <Para2>We use cookies on our website. A cookie is a small file stored by your browser on your device.</Para2>
    <Para2><strong>Necessary cookies:</strong> Help us recognize you on return visits and remember your preferences and consent choices.</Para2>
    <Para2><strong>Session cookies:</strong> Track which parts of the website you view during your visit. Automatically deleted when you close your browser.</Para2>
    <Para2><strong>Tracking cookies:</strong> With your consent, used to build a profile of your online behavior to show relevant content. Not linked to your personal details.</Para2>
    <Para2><strong>Google Analytics:</strong> Used to track and receive reports on how visitors use the website. Information is anonymized as much as possible.</Para2>
    <Para2><strong>Social media:</strong> Our website features buttons from Facebook, Instagram, YouTube and other platforms. These place their own cookies. Read their respective privacy statements for details.</Para2>

    <Para2 style={{marginTop:24,fontStyle:"italic",color:cc.muted}}>This Privacy Statement was last amended on August 2025.</Para2>
    </FadeIn>
  </div><Footer/></div>);
}

class ErrorBoundary extends React.Component{constructor(p){super(p);this.state={error:null};}static getDerivedStateFromError(e){return{error:e};}render(){if(this.state.error)return React.createElement("div",{style:{padding:40,fontFamily:"monospace",color:"red",whiteSpace:"pre-wrap"}},"Error: "+this.state.error.message+"\n\n"+this.state.error.stack);return this.props.children;}}
...
export default function App(){
  ...
  return(<ErrorBoundary><AppContext.Provider value={ctx}>
    <style>{`*,*::before,*::after{box-sizing:border-box;margin:0}body{margin:0;overflow-x:hidden}::selection{background:${P};color:white}@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}@keyframes fadeSlide{0%{opacity:0;transform:translateY(12px)}100%{opacity:1;transform:translateY(0)}}@keyframes slideIn{from{transform:translateX(100px);opacity:0}to{transform:translateX(0);opacity:1}}@media(max-width:768px){.hero-grid,.detail-grid,.invest-grid,.listings-cities-grid{grid-template-columns:1fr!important}.hero-img{display:none!important}.nav-desktop{display:none!important}.nav-mobile-btn{display:block!important}.listings-grid{grid-template-columns:1fr!important}.form-grid{grid-template-columns:1fr!important}.details-grid{grid-template-columns:1fr 1fr!important}.listings-map-grid{grid-template-columns:1fr!important}.listings-map-col{display:none!important}.invest-images{grid-template-columns:1fr 1fr!important}section{padding:60px 16px!important}.booking-card{position:relative!important;top:auto!important}.hero-section{padding:120px 16px 60px!important}}@media(max-width:480px){.details-grid{grid-template-columns:1fr!important}.home-listings-grid{grid-template-columns:1fr!important}.steps-grid{grid-template-columns:1fr!important}.stats-grid{grid-template-columns:1fr 1fr!important}.cities-grid{grid-template-columns:1fr 1fr!important}.reviews-grid{grid-template-columns:1fr!important}}@media(min-width:769px){.nav-mobile-btn{display:none!important}}`}</style>
    <Navbar/>{toast&&<Toast message={toast.message} type={toast.type} onClose={()=>setToast(null)}/>}
    {page==="home"&&<HomePage/>}{page==="listings"&&<ListingsPage initialCity={pp.city} initialSearch={pp.search}/>}{page==="flat"&&<FlatDetailPage flatId={pp.id}/>}{page==="admin"&&<AdminPage/>}{page==="investors"&&<InvestorsPage/>}{page==="about"&&<AboutPage/>}{page==="faq"&&<FAQPage/>}{page==="profile"&&<ProfilePage/>}{page==="terms"&&<TermsPage/>}{page==="privacy"&&<PrivacyPage/>}
