...
function HomePage(){
  const{navigate,cities,flats}=useContext(AppContext);const[ci,setCi]=useState(0);const[cityFade,setCityFade]=useState(true);const[heroSearch,setHeroSearch]=useState('');const[showCities,setShowCities]=useState(false);const cn=["Nicosia","Athens","Limassol"];
  ...
  useEffect(()=>{const t=setInterval(()=>{setCityFade(false);setTimeout(()=>{setCi(i=>(i+1)%cn.length);setCityFade(true);},400);},3200);return()=>clearInterval(t);},[]);
...
export default function App(){
  ...
  return(<ErrorBoundary><AppContext.Provider value={ctx}>
    <style>{`*,*::before,*::after{box-sizing:border-box;margin:0}body{margin:0;overflow-x:hidden}::selection{background:${P};color:white}@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}@keyframes fadeSlide{0%{opacity:0;transform:translateY(12px)}100%{opacity:1;transform:translateY(0)}}@keyframes slideIn{from{transform:translateX(100px);opacity:0}to{transform:translateX(0);opacity:1}}@media(max-width:768px){.hero-grid,.detail-grid{grid-template-columns:1fr!important}.hero-img{display:none!important}.nav-desktop{display:none!important}.nav-mobile-btn{display:block!important}.listings-grid{grid-template-columns:1fr!important}.form-grid{grid-template-columns:1fr!important}.details-grid{grid-template-columns:1fr 1fr!important}.listings-map-grid{grid-template-columns:1fr!important}.listings-map-col{display:none!important}.listings-cities-grid{grid-template-columns:1fr!important}}@media(min-width:769px){.nav-mobile-btn{display:none!important}}`}</style>
    <Navbar/>{toast&&<Toast message={toast.message} type={toast.type} onClose={()=>setToast(null)}/>}
    {page==="home"&&<HomePage/>}{page==="listings"&&<ListingsPage initialCity={pp.city} initialSearch={pp.search}/>}{page==="flat"&&<FlatDetailPage flatId={pp.id}/>}{page==="admin"&&<AdminPage/>}{page==="investors"&&<InvestorsPage/>}{page==="about"&&<AboutPage/>}{page==="faq"&&<FAQPage/>}{page==="terms"&&<TermsPage/>}{page==="privacy"&&<PrivacyPage/>}
