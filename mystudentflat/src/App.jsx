...
function Modal({children,onClose}){return(<div style={{position:"fixed",inset:0,zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:24}} onClick={onClose}><div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(4px)"}}/><div style={{position:"relative",background:cc.white,borderRadius:20,padding:32,maxWidth:480,width:"100%",maxHeight:"90vh",overflowY:"auto",boxShadow:"0 24px 48px rgba(0,0,0,0.15)"}} onClick={e=>e.stopPropagation()}><button onClick={onClose} style={{position:"absolute",top:16,right:16,background:"none",border:"none",fontSize:20,cursor:"pointer",color:cc.muted}}>✕</button>{children}</div></div>);}
function Toast({message,type="success",onClose}){useEffect(()=>{const t=setTimeout(onClose,3500);return()=>clearTimeout(t);},[onClose]);return<div style={{position:"fixed",top:20,right:20,zIndex:9999,padding:"14px 24px",borderRadius:12,background:type==="success"?Pd:cc.red,color:"#fff",fontFamily:ff,fontSize:14,fontWeight:500,boxShadow:"0 8px 30px rgba(0,0,0,0.2)",animation:"slideIn 0.3s ease"}}>{type==="success"?"✓":"✗"} {message}</div>;}
...
function Navbar(){
  ...
  return(<><nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,background:scrolled?"rgba(255,255,255,0.96)":"rgba(255,255,255,0.9)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${scrolled?cc.border:"transparent"}`,transition:"all 0.3s",padding:scrolled?"8px 0":"14px 0"}}>
    <div style={{maxWidth:1200,margin:"0 auto",padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      ...
      <button className="nav-mobile-btn" onClick={()=>setMenuOpen(!menuOpen)} style={{display:"none",background:"none",border:"none",fontSize:22,cursor:"pointer",color:cc.text}}>{menuOpen?"✕":"☰"}</button>
    </div>
...
</div><div style={{borderTop:`1px solid ${cc.border}`,paddingTop:20,display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:12}}><span style={{fontFamily:ff,fontSize:12,color:cc.muted}}>Copyright © by My Student Flat</span><div style={{display:"flex",gap:16}}>{["Terms & Conditions","Privacy Policy"].map(l=><a key={l} href="#" style={{fontFamily:ff,fontSize:12,color:cc.muted,textDecoration:"none"}}>{l}</a>)}</div></div></div></footer>);}

function HomePage(){
  const{navigate,cities}=useContext(AppContext);const[ci,setCi]=useState(0);const cn=["Nicosia","Athens","Limassol"];
  useEffect(()=>{const t=setInterval(()=>setCi(i=>(i+1)%cn.length),2800);return()=>clearInterval(t);},[]);
  const STEPS=[{t:"Browse Our Flats",d:"Explore verified listings and filter by availability, price, location, and proximity to your university.",i:"01"},{t:"Book Hassle-Free",d:"Book online and secure your future student flat.",i:"02"},{t:"E-Sign The Agreement",d:"Review and sign all the necessary documents digitally.",i:"03"},{t:"Ready To Move In",d:"Grab the keys and enjoy your flat.",i:"04"}];
  ...
  return(<div>
    ...
    <section style={{padding:"90px 24px",background:"#fff"}}><div style={{maxWidth:1200,margin:"0 auto"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(240px, 1fr))",gap:24}}>{STEPS.map((s,i)=>(<FadeIn key={i} delay={i*0.06}><div style={{padding:28,borderRadius:16,background:"#fff",border:`1px solid ${cc.border}`,height:"100%",transition:"all 0.3s",textAlign:"center"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-4px)";e.currentTarget.style.boxShadow="0 12px 32px rgba(67,56,202,0.08)";e.currentTarget.style.borderColor=P;}} onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";e.currentTarget.style.borderColor=cc.border;}}>
        <div style={{width:56,height:56,borderRadius:"50%",background:cc.primaryLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:700,fontFamily:ff,color:P,margin:"0 auto 16px"}}>{s.i}</div>
    ...
    <section style={{padding:"90px 24px",background:"#fff"}}><div style={{maxWidth:1200,margin:"0 auto"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(300px, 1fr))",gap:20}}>{REVIEWS.map((r,i)=>(<FadeIn key={i} delay={i*0.06}><div style={{padding:28,borderRadius:16,background:"#fff",border:`1px solid ${cc.border}`,height:"100%"}}>
        <div style={{color:P,fontSize:24,marginBottom:6}}>"</div><p style={{fontFamily:ff,fontSize:14,lineHeight:1.7,color:cc.text,margin:"0 0 16px",fontStyle:"italic"}}>{r.t}</p>
        ...
        <div style={{color:"#f59e0b",fontSize:13,marginTop:8}}>— 5/5</div>
      </div></FadeIn>))}</div>
    </div></section>
...
function ListingsPage({initialCity}){
  ...
  return(<div style={{paddingTop:80,minHeight:"100vh",background:"#fff"}}><div style={{maxWidth:1200,margin:"0 auto",padding:"40px 24px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(320px, 1fr))",gap:22}} className="listings-grid">{filtered.map((fl,i)=>(<FadeIn key={fl.id} delay={i*0.04}><div onClick={()=>navigate("flat",{id:fl.id})} style={{borderRadius:16,background:"#fff",border:`1px solid ${cc.border}`,overflow:"hidden",cursor:"pointer",transition:"all 0.3s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-4px)";e.currentTarget.style.boxShadow="0 14px 44px rgba(0,0,0,0.08)";}} onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";}}>
      <div style={{position:"relative",aspectRatio:"16/10",overflow:"hidden"}}><img src={fl.cover_url||"https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?w=800&h=500&fit=crop"} alt={fl.title} style={{width:"100%",height:"100%",objectFit:"cover",transition:"transform 0.4s"}} onMouseEnter={e=>e.target.style.transform="scale(1.04)"} onMouseLeave={e=>e.target.style.transform=""}/><div style={{position:"absolute",top:12,left:12,display:"flex",gap:6}}><Tag>Available Now</Tag>{fl.is_featured&&<Tag color={P} bg={cc.primaryLight}>Featured</Tag>}</div></div>
      <div style={{padding:"16px 20px 20px"}}><h3 style={{fontFamily:fs,fontSize:17,color:cc.text,margin:"0 0 2px"}}>{fl.title}</h3><p style={{fontFamily:ff,fontSize:13,color:cc.muted,margin:"0 0 12px"}}>{fl.address}</p><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:12,borderTop:`1px solid ${cc.border}`}}><div style={{display:"flex",gap:12}}><span style={{fontFamily:ff,fontSize:13,color:cc.muted}}>{fl.bedrooms}BR</span>{fl.sqm&&<span style={{fontFamily:ff,fontSize:13,color:cc.muted}}>{fl.sqm}m²</span>}</div><div style={{fontFamily:fs,fontSize:20,color:P}}>€{fl.price_per_month}<span style={{fontSize:11,color:cc.muted,fontFamily:ff,fontWeight:400}}>/month</span></div></div></div>
    </div></FadeIn>))}</div>
...
function FlatDetailPage({flatId}){
  const{flats,user,navigate,images,showToast}=useContext(AppContext);const[showAuth,setShowAuth]=useState(false);const[ld,setLd]=useState("12");const[mid,setMid]=useState("");const[success,setSuccess]=useState(false);const[imgIdx,setImgIdx]=useState(0);const[booking,setBooking]=useState(false);
  ...
  return(<div style={{paddingTop:80,minHeight:"100vh",background:"#fff"}}>{showAuth&&<AuthModal onClose={()=>setShowAuth(false)}/>}<div style={{maxWidth:1200,margin:"0 auto",padding:"32px 24px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 380px",gap:32,alignItems:"start"}} className="detail-grid">
      <div>
        ...
        <FadeIn delay={0.06}><div style={{display:"flex",gap:8,marginBottom:14}}><StatusBadge status={flat.status}/>{flat.is_featured&&<Tag color={P} bg={cc.primaryLight}>Featured</Tag>}</div><h1 style={{fontFamily:fs,fontSize:28,color:cc.text,margin:"0 0 6px"}}>{flat.title}</h1><p style={{fontFamily:ff,fontSize:15,color:cc.muted,margin:"0 0 20px"}}>{flat.address}</p><p style={{fontFamily:ff,fontSize:15,lineHeight:1.7,color:cc.text,margin:"0 0 28px"}}>{flat.description}</p></FadeIn>
        <FadeIn delay={0.1}><h3 style={{fontFamily:fs,fontSize:20,color:cc.text,marginBottom:12}}>Details</h3><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:28}} className="details-grid">{[["—","Beds",flat.bedrooms],["—","Baths",flat.bathrooms],["—","Area",`${flat.sqm||"—"}m²`],["—","Floor",flat.floor||"—"],["—","Balcony",flat.has_balcony?"Yes":"No"],["—","Uni",flat.distance_to_university||"—"]].map(([i,l,v])=><div key={l} style={{padding:"12px 14px",borderRadius:10,background:cc.light,display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:18}}>{i}</span><div><div style={{fontFamily:ff,fontSize:11,color:cc.muted}}>{l}</div><div style={{fontFamily:ff,fontSize:14,fontWeight:600,color:cc.text}}>{v}</div></div></div>)}</div></FadeIn>
        <FadeIn delay={0.14}><h3 style={{fontFamily:fs,fontSize:20,color:cc.text,marginBottom:12}}>Amenities</h3><div style={{display:"flex",flexWrap:"wrap",gap:8}}>{flat.amenities?.map(a=><span key={a} style={{display:"inline-flex",alignItems:"center",gap:4,padding:"6px 14px",borderRadius:100,background:cc.light,fontSize:13,fontFamily:ff,color:cc.text,border:`1px solid ${cc.border}`}}>{a}</span>)}</div></FadeIn>
      </div>
      <FadeIn delay={0.08}><div style={{position:"sticky",top:100,padding:26,borderRadius:18,background:"#fff",border:`1px solid ${cc.border}`,boxShadow:"0 4px 20px rgba(0,0,0,0.04)"}}>
        <div style={{marginBottom:22}}><span style={{fontFamily:fs,fontSize:32,color:P}}>€{flat.price_per_month}</span><span style={{fontFamily:ff,fontSize:14,color:cc.muted}}>/month</span></div>
        <div style={{marginBottom:16}}><label style={lbl}>Lease Duration</label><div style={{display:"flex",gap:8}}>{(flat.available_lease_durations||["12","24"]).map(d=><button key={d} onClick={()=>setLd(d)} style={{flex:1,padding:"12px",borderRadius:10,border:`2px solid ${ld===d?P:cc.border}`,background:ld===d?P:"#fff",color:ld===d?"#fff":cc.text,fontFamily:ff,fontSize:14,fontWeight:600,cursor:"pointer",transition:"all 0.2s"}}>{d} months</button>)}</div></div>
        <div style={{marginBottom:18}}><label style={lbl}>Move-in Date</label><input type="date" value={mid} min={flat.earliest_move_in||new Date().toISOString().split("T")[0]} onChange={e=>setMid(e.target.value)} style={inp}/></div>
        {mid&&<div style={{padding:14,borderRadius:10,background:cc.light,marginBottom:18}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontFamily:ff,fontSize:13,color:cc.muted}}>Move-out</span><span style={{fontFamily:ff,fontSize:13,fontWeight:600,color:cc.text}}>{mod}</span></div><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontFamily:ff,fontSize:13,color:cc.muted}}>Total</span><span style={{fontFamily:ff,fontSize:13,fontWeight:600,color:cc.text}}>€{(flat.price_per_month*parseInt(ld)).toLocaleString()}</span></div></div>}
        <button onClick={handleBook} disabled={booking} style={{...btn(),width:"100%",justifyContent:"center",padding:"15px",fontSize:16,borderRadius:10,opacity:booking?0.6:1}}>{booking?"Submitting...":user?"Book This Flat":"Sign In to Book"}</button>
        <p style={{textAlign:"center",fontFamily:ff,fontSize:12,color:cc.muted,marginTop:10}}>No payment now. We confirm availability first.</p>
      </div></FadeIn>
    </div>)}
  </div><Footer/></div>);
}
...
function AdminPage(){
  ...
  if(editing){const flatImgs=editing!=="new"?images.filter(i=>i.flat_id===editing).sort((a,b)=>a.sort_order-b.sort_order):[];
    return(<div style={{paddingTop:80,minHeight:"100vh",background:cc.light}}><div style={{maxWidth:900,margin:"0 auto",padding:"32px 24px"}}><div style={{padding:28,borderRadius:16,background:"#fff",border:`1px solid ${cc.border}`}}>
      ...
      <div style={{display:"flex",gap:16,marginBottom:20,flexWrap:"wrap"}}>{[["has_balcony","Balcony"],["is_furnished","Furnished"],["is_featured","Featured"]].map(([k,l])=><label key={k} style={{display:"flex",alignItems:"center",gap:8,fontFamily:ff,fontSize:14,cursor:"pointer"}}><input type="checkbox" checked={formData[k]} onChange={e=>set(k,e.target.checked)}/>{l}</label>)}</div>
  ...
  return(<div style={{paddingTop:80,minHeight:"100vh",background:cc.light}}><div style={{maxWidth:1200,margin:"0 auto",padding:"32px 24px"}}>
    ...
    {tab==="overview"&&<><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))",gap:16,marginBottom:32}}>{stats.map(s=><div key={s.l} style={{padding:24,borderRadius:14,background:"#fff",border:`1px solid ${cc.border}`,display:"flex",alignItems:"center",gap:16}}><div style={{fontSize:28}}>{s.i}</div><div><div style={{fontFamily:fs,fontSize:28,color:cc.text}}>{s.v}</div><div style={{fontFamily:ff,fontSize:13,color:cc.muted}}>{s.l}</div></div></div>)}</div>
    ...
    {tab==="flats"&&<div style={{borderRadius:14,background:"#fff",border:`1px solid ${cc.border}`,overflow:"hidden"}}><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontFamily:ff,fontSize:14}}><thead><tr style={{borderBottom:`2px solid ${cc.border}`}}>{["","Flat","City","Price","Beds","Status","Views","Actions"].map(h=><TH key={h}>{h}</TH>)}</tr></thead><tbody>{flats.map(fl=><tr key={fl.id} style={{borderBottom:`1px solid ${cc.border}`}}>
      ...
      <td style={{padding:"10px 16px",fontWeight:600,color:cc.text}}>{fl.title}{fl.is_featured&&<span style={{marginLeft:6,color:"#f59e0b"}}>*</span>}</td>
      ...
      <td style={{padding:"10px 16px"}}><div style={{display:"flex",gap:6}}><button onClick={()=>startEdit(fl)} style={{padding:"6px 12px",borderRadius:6,border:`1px solid ${cc.border}`,background:"#fff",fontFamily:ff,fontSize:12,cursor:"pointer",color:cc.text}}>Edit</button><button onClick={()=>setConfirmDel(fl)} style={{padding:"6px 12px",borderRadius:6,border:"1px solid #fecaca",background:"#fef2f2",fontFamily:ff,fontSize:12,cursor:"pointer",color:cc.red}}>Delete</button></div></td>
...
export default function App(){
  ...
  if(loading)return<div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"#fff"}}><div style={{textAlign:"center"}}><img src={LOGO} alt="Loading" style={{height:40,marginBottom:16,animation:"pulse 1.5s infinite"}}/><p style={{fontFamily:ff,color:cc.muted}}>Loading...</p></div></div>;
  return(<AppContext.Provider value={ctx}>
    ...
    {page==="home"&&<HomePage/>}{page==="listings"&&<ListingsPage initialCity={pp.city}/>}{page==="flat"&&<FlatDetailPage flatId={pp.id}/>}{page==="admin"&&<AdminPage/>}{page==="investors"&&<InvestorsPage/>}{page==="about"&&<AboutPage/>}{page==="faq"&&<FAQPage/>}
  </AppContext.Provider>);
}
