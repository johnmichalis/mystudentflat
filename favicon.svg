  ...
  return(<div style={{paddingTop:80,minHeight:"100vh",background:"#fff"}}>{showAuth&&<AuthModal onClose={()=>setShowAuth(false)}/>}<div style={{maxWidth:1200,margin:"0 auto",padding:"32px 24px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 380px",gap:32,alignItems:"start"}} className="detail-grid">
      <div>
        ...
        {/* Map */}
        {(flat.latitude&&flat.longitude)&&<FadeIn delay={0.26}><h3 style={{fontFamily:fs,fontSize:20,color:cc.text,marginBottom:12,marginTop:28}}>Location</h3>
        <div style={{borderRadius:14,overflow:"hidden",border:`1px solid ${cc.border}`,aspectRatio:"16/9"}}>
          <iframe src={`https://www.openstreetmap.org/export/embed.html?bbox=${flat.longitude-0.005}%2C${flat.latitude-0.003}%2C${flat.longitude+0.005}%2C${flat.latitude+0.003}&layer=mapnik&marker=${flat.latitude}%2C${flat.longitude}`} style={{width:"100%",height:"100%",border:"none"}}/>
        </div>
        <a href={`https://www.google.com/maps?q=${flat.latitude},${flat.longitude}`} target="_blank" rel="noopener noreferrer" style={{display:"inline-flex",alignItems:"center",gap:6,marginTop:8,fontFamily:ff,fontSize:13,color:P,textDecoration:"none",fontWeight:500}}><MapPin size={14}/>View on Google Maps</a>
        </FadeIn>}
      ...
      <FadeIn delay={0.08}><div style={{position:"sticky",top:100,padding:26,borderRadius:18,background:"#fff",border:`1px solid ${cc.border}`,boxShadow:"0 4px 20px rgba(0,0,0,0.04)"}}>
        ...
        <div style={{marginBottom:18}}><label style={lbl}>Move-in Date</label>{flat.earliest_move_in&&flat.latest_move_in&&<p style={{fontFamily:ff,fontSize:12,color:cc.muted,margin:"0 0 6px"}}>Available: {flat.earliest_move_in} to {flat.latest_move_in}</p>}<input type="date" value={mid} min={flat.earliest_move_in||new Date().toISOString().split("T")[0]} max={flat.latest_move_in||""} onChange={e=>setMid(e.target.value)} style={inp}/></div>
...
function AdminPage(){
  ...
  return(<div style={{paddingTop:80,minHeight:"100vh",background:cc.light}}><div style={{maxWidth:1200,margin:"0 auto",padding:"32px 24px"}}>
    ...
    {tab==="flats"&&<div style={{borderRadius:14,background:"#fff",border:`1px solid ${cc.border}`,overflow:"hidden"}}><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontFamily:ff,fontSize:14}}><thead><tr style={{borderBottom:`2px solid ${cc.border}`}}>{["","Flat","City","Price","Beds","Status","Views","Actions"].map(h=><TH key={h}>{h}</TH>)}</tr></thead><tbody>{flats.map(fl=><tr key={fl.id} style={{borderBottom:`1px solid ${cc.border}`}}>
      ...
      <td style={{padding:"10px 16px",fontWeight:600,color:cc.text}}>{fl.title}{fl.is_featured&&<Star size={14} style={{marginLeft:6,display:"inline"}} fill="#f59e0b" color="#f59e0b"/>}</td>
